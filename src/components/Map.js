...
export default class Maps {
    constructor(container, app) {
        ...
        this.svBearing = 180;
        this.map = L.map('map',{ zoomControl: false }).setView([42.36, -83.1], 12);
        this.init(this);
    }

    init(_map){
      new L.Control.Zoom({ position: 'topright' }).addTo(_map.map);
      let parcelLayer = Vector.vectorTileLayer("https://tiles.arcgis.com/tiles/qvkbeam7Wirps6zC/arcgis/rest/services/parcels/VectorTileServer", {
        minZoom: 15,
        simplifyFactor: 0.35,
        precision: 5,
        style: (style) => {
    ...
    changeVisibility(layers, visibility, _map){
      layers.forEach(layer => {
        _map.map.eachLayer(function(_layer){
          console.log(_layer);
        });
